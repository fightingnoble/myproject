...
class crxb_Conv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        if self.bias is not None:
            bias_quan = quantize_weight(self.bias, self.delta_y) * 1
            a = bias_quan.unsqueeze(1).unsqueeze(1)
            output += a
        output = F.hardtanh(output, min_val=-self.h_lvl - 1, max_val=self.h_lvl)
...
class crxb_Linear(nn.Linear):
    ...
    def forward(self, input):
        ...
        weight_padded = F.pad(weight_quan, self.w_pad,
                              mode='constant', value=0)
        input_padded = F.pad(input, self.input_pad,
                             mode='constant', value=0)
        # 2.3. reshape
...
# 2020/03/15: remove the process of multiply scalar to the activation between two layer's ADC to DAC
#             Now the data delivered between two layer is only integer!!
