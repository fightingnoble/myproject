...
def Quantizer_train(x, basis, levels, thrs, level_codes_channelwise,
    ...
    Args:
        ...
        levels (torch.Tensor):
        thrs (torch.Tensor):
        level_codes_channelwise (torch.Tensor):
    ...
    * ``basis``: basis of quantized weight.
    ...
    if training:
        for _ in torch.arange(N):
            ...
            # --------------simple notation-----------
            # bits_y [K x K x iC, oc, lbit] basis [lbit, oc] reshape_x [K x K x iC, oC]
            # B: [oC, K x K x iC, lbit], v: [oC, lbit, 1], w: [oC, K x K x iC, 1]
            B = bits_y.permute(1, 0, 2).detach().cpu().numpy()
            BT = bits_y.permute(1, 2, 0).detach().cpu().numpy()
            v = basis.permute(1, 0).detach().cpu().numpy()
            w = reshape_x.permute(1, 0).detach().cpu().numpy()
            # --------------simple notation-----------

            # # [num_levels-1, num_levels] x [num_levels, lbit] =>
            # # [num_levels-1, lbit]
            # G1 = torch.matmul(variation_multiplier, level_codes).cpu().numpy()
            # zero_G1 = np.zeros_like(G1)
            # G1 = np.concatenate((np.concatenate((G1,zero_G1),1), np.concatenate((zero_G1,G1),1)))
            #
            # def ineq_G1(basis):
            #     """
            #     Quantize weight.
            #     Args:
            #         basis (np.array): a 2D tensor. [lbit,]
            #     """
            #     # G1 (np.array): a 2D tensor. [num_levels-1, lbit]
            #     return np.matmul(G1, basis)
            #
            # def ineq_G1_jac(basis):
            #     """
            #     Quantize weight.
            #     Args:
            #         basis (np.array): a 2D tensor. [lbit,]
            ...
            #     return G1
            #
            # bounds = Bounds(Gmin, Gmax)
            #
            # # [num_levels-1, num_levels] x []
            # ineq_cons = {'type': 'ineq',
            #              'fun': ineq_G1,
            #              'jac': ineq_G1_jac}
            # # linear_constraint = LinearConstraint(G1, 0, np.inf)

            new_basis = []
            loss = 0
            for i in range(oc):
                def f_loss(x):
                    F = np.matmul(B[i], x) - w[i]
                    loss1 = 1 / 2 * np.sum(F ** 2)
                    return loss1.item()

                def jac_loss(x):
                    F = np.matmul(B[i], x) - w[i]
                    return np.matmul(B[i].T, F)

                def hess_loss(x):
                    return np.matmul(B[i].T, B[i])

                res = minimize(f_loss, v[i], method='BFGS', jac=jac_loss, options={'disp': True})
                new_basis.append(res.x)
                print(res.x)
                loss += f_loss(v[i])
            print('f_loss:', loss)

            new_basis = torch.tensor(new_basis).permute(1, 0).to(device)
...
def reshaped_QuantizedWeight(x, n, nbit=2, training=False):
    ...
    * ``basis``: basis of quantized weight.
    ...
    basis_t, levels_t, thrs_t, level_codes_t = Quantizer_train(x.detach(), basis, levels, thrs, level_codes_channelwise,
                                                               level_codes, thrs_multiplier,
                                                               nbit, training)
    # alpha = alpha.detach()
...
if __name__ == "__main__":
    ...
    wq, levels, threholds = reshaped_QuantizedWeight(w_norm, OC * K1 * K2, 4, True)
