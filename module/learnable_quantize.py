...
def Quantizer_train(x, basis, level_codes, thrs_multiplier, nbit=2, training=False):
    """
    Quantize weight.
    Args:
        x (torch.Tensor): a 4D tensor. [K x K x iC x oC] -> [oC x iC x K x K]
    ...
    * ``basis``: basis of quantized weight.
    ...
    if training:
        for _ in torch.arange(N):
            ...
            # calculate BTxB
            # [oC,nbit+1, K x K x iC] x [oC, K x K x iC,nbit+1] => [oC,nbit+1,nbit+1]
            BTxB = torch.matmul(bits_y.permute(1, 2, 0), bits_y.permute(1, 0, 2)) + delta * torch.eye(nbit + 1,
                                                                                                      device=device)
            # calculate inverse of BTxB
            # [oC,nbit+1,nbit+1]
            if nbit + 1 > 2:
                ...
                det = torch.det(BTxB)
                BTxB_inv = torch.stack((BTxB[:, 1, 1], -BTxB[:, 0, 1], -BTxB[:, 1, 0], BTxB[:, 0, 0]),
                                       1).view(OC, nbit + 1, nbit + 1) / det.unsqueeze(-1).unsqueeze(-1)
            elif nbit + 1 == 1:
                BTxB_inv = 1 / BTxB
            else:
                BTxB_inv = None
            ...
            # calculate BTxX
            # bits_y [K x K x iC, oc, nbit + 1] reshape_x [K x K x iC, oC]
            # [oC, nbit + 1, K x K x iC] [oC, K x K x iC, 1] => [oC, nbit + 1, 1]
            BTxX = torch.matmul(bits_y.permute(1, 2, 0), reshape_x.permute(1, 0).unsqueeze(-1))
            BTxX = BTxX + (delta * basis.permute(1, 0).unsqueeze(-1))  # + basis

            # calculate new basis
            # BTxB_inv: [oC, nbit + 1, nbit + 1] BTxX: [oC, nbit + 1, 1]
            # [oC, nbit + 1, nbit + 1] x [oC, nbit + 1, 1] => [oC, nbit + 1, 1] => [nbit + 1, oC]
            new_basis = torch.matmul(BTxB_inv, BTxX).squeeze(-1).permute(1, 0)
            # print(BTxB_inv.shape,BTxX.shape)
            # print(new_basis.shape)
