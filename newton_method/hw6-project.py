...
# BFGS
Broyden_k = Hessian_logistic_regression
Goldfarb_k = np.linalg.pinv(Hessian_logistic_regression)
unit_matrix = np.diag(np.ones(23))
...
while gradient_norm >= 1e-5:
    # KKT
    # F_temp = - np.dot(A_cons, Goldfarb_k)
    # F = np.dot(F_temp, A_cons.T)
    # F = np.linalg.inv(F)
    #
    # E_temp = np.dot(Goldfarb_k, A_cons.T)
    # E = np.dot(E_temp, F)
    #
    # C = np.dot(E_temp, F)
    # C = Goldfarb_k - np.dot(C, F_temp)
    #
    # inv_Hessian_lagrange_temp1 = np.concatenate((C, E), axis=1)
    # inv_Hessian_lagrange_temp2 = np.concatenate((E.T, F), axis=1)
    # inv_Hessian_lagrange = np.concatenate((inv_Hessian_lagrange_temp1, inv_Hessian_lagrange_temp2),
    #                                       axis=0)
    ...
    Hessian_lagrange_t1 = np.concatenate((Broyden_k, A_cons.T), axis=1)
    ...
    grad_logistic_regression = np.dot(x_hom, (logistic_regression - y))
    delta_grad_logistic_regression = grad_logistic_regression - grad_logistic_regression_k

    # Goldfarb_k_temp1 = delta_w.T.dot(delta_grad_logistic_regression)
    # Goldfarb_k_temp2 = unit_matrix - delta_w.dot(delta_grad_logistic_regression.T) / Goldfarb_k_temp1
    # Goldfarb_k_temp3 = delta_w.dot(delta_w.T)
    #
    # Goldfarb_k = Goldfarb_k_temp2.dot(Goldfarb_k).dot(Goldfarb_k_temp2.T) + Goldfarb_k_temp3 / Goldfarb_k_temp1

    Broyden_k_t1 = np.dot(delta_grad_logistic_regression,delta_grad_logistic_regression.T)/np.dot(delta_grad_logistic_regression.T,delta_w)
    Broyden_k_t2 = (Broyden_k.dot(delta_w).dot(delta_w.T).dot(Broyden_k))/(delta_w.T.dot(Broyden_k).dot(delta_w))
    Broyden_k = Broyden_k + Broyden_k_t1-Broyden_k_t2
...
plt.title("""BFGS method on Logistic Regression with balanced data
...
plt.savefig('BFGS_on_nba_data_with_balanced_data.png')
plt.show()
